class ScrapeOddsJob < ApplicationJob
    queue_as :default
    def perform
        def initialize
        ...
        def get_tables
          @events = Event.where(status: "UPCOMING").or(Event.where(status: "INPROGRESS"))
